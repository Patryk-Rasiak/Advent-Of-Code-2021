data = open("advent/17/17a.txt").read()[13:]
xs, ys = data.split(", ")
...
def solve(tx1, tx2, ty1, ty2):
    S = [0, 0]
    # (velocity_of_x, velocity_of_y): max height
    hits = {}

    for vel_x in range(1000):
        for vel_y in range(1000):
            S = [0, 0]
    ...
    best_x, best_y = max(hits, key=hits.get)

    # Return velocities with maximum height and the height
    return (best_x, best_y), hits[(best_x, best_y)]
