...
data = [[int(x) for x in line.strip()] for line in open("advent/15/15b.txt")]
...
def scale(data, K):
    N = len(data)
    result = [[0 for _ in range(K*N)] for _ in range(K*N)]
    for ii in range(K):
        for jj in range(K):
            for i in range(N):
                for j in range(N):
                    result[ii*N+i][jj*N+j] = (data[i][j]-1+ii+jj) % 9+1
    return result


print(solve(scale(data, 5)))
